...
def main() -> None:
    ...
    print("group\tcounty\tfips\tpop2020\tpop2023\tpop_change_pct\tforeign_born_pct_acs5_2023\tforeign_born_entered_2010plus_pct\trucc\thpsa\tretail_est_per_10k\thealth_est_per_10k\tfood_est_per_10k")
    for group, rows in (("lower_change", selected[: args.n]), ("higher_change", selected[args.n :])):
        for change, key in rows:
            ...
            print("\t".join([group, names[key], key, str(pop20), str(pop23), f"{change:.2f}", foreign_born, recent, str(rucc[key]), "yes" if key in hpsa else "no", *values]))

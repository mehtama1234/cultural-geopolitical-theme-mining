#!/usr/bin/env python3
"""Weighted SHED care-by-age adaptation comparison."""
...
def main() -> None:
    ...
    required = {"weight", "CG4", "ppagecat"} | {field for field, _ in METRICS.values()}
    ...
    for row in rows:
        care, age = clean(row.get("CG4")), clean(row.get("ppagecat"))
        if care and age:
            cells[care][age].append(row)
    result: dict[str, object] = {
        "format": "us-shed-care-age-distribution-v1",
        ...
        "cells": {
            care: {
                age: {metric: metric_share(group, field, yes_values)
                      for metric, (field, yes_values) in METRICS.items()}
                for age, group in sorted(age_groups.items())
            }
            for care, age_groups in sorted(cells.items())
        },
    }

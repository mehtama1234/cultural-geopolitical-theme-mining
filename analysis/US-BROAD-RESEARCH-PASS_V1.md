...
The first such diagnostic is now the [SIPP Fay-BRR race × resource layer](projects/us-household-calendar-integration/sipp-fay-brr-race-resource-layer-v1.md). It uses the full 2025 SIPP person-month slice and 240 replicate weights to compare utility pressure, hunger, and one-job status across four race recodes and four resource bands. It supplies distributional uncertainty and applies the selected variables’ documented status flags; full domain-universe reconstruction, causal mechanisms, and downstream outcomes remain open.

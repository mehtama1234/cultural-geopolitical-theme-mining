# Source record: Kenya content-moderation court dispute

**Checked:** 2026-09-11  
**Source family:** official court record  
**Use:** contested-case check for the Kenyan content-moderation case in the ILO paper

## Primary record

- [Samasource EPZ Limited t/a Sama v Meta Platforms, Incorporated & 186 others, Kenya Court of Appeal judgment, 20 September 2024](https://new.kenyalaw.org/akn/ke/judgment/keca/2024/1152/eng%402024-09-20/source), accessed 2026-09-11.
- [Meta Platforms, Inc & another v Motaung & another, Kenya Court of Appeal judgment, 20 September 2024](https://new.kenyalaw.org/akn/ke/judgment/keca/2024/1262/eng%402024-09-20/source), accessed 2026-09-11.

The record is an appellate judgment about interim orders. It is not a final ruling on every employment, health, discrimination, or union claim.

## What the court record establishes

- The underlying dispute involved content moderators working through Sama and claims involving Meta, including employment status, termination, working conditions, and mental-health care.
- The trial court had issued interim orders concerning termination, contractual terms, replacement hiring, care, and immigration status while the petition was pending.
- The Court of Appeal held that the interim mandatory orders could effectively determine parts of the dispute before a full hearing.
- On 20 September 2024, the Court of Appeal allowed the consolidated appeals, set aside the 2 June 2023 ruling and consequential orders, and substituted an order dismissing the relevant application.
- In a separate but related judgment on the same date, the Court of Appeal dismissed Meta’s appeals concerning jurisdiction and the continuation of interim orders, finding those appeals without merit and awarding costs to the respondents.

## What it does not establish

- It does not decide whether Meta was the employer on the merits.
- It does not decide whether the workers’ health, pay, termination, discrimination, or organizing claims were true.
- It does not show that the workers’ underlying petition ended in defeat; it shows that the interim protections were removed on appeal.
- It does not measure the later employment, health, or bargaining outcome.

## Why this is a useful contested case

```text
reach the lead firm in court -> obtain interim protection -> win a durable remedy
```

The first two links appear in the record; the third remains open. This is direct evidence that cross-border legal reach does not automatically become stable worker power.

## Coding

| Field | Coding |
|---|---|
| Level | Cross-border contractor and lead-firm dispute |
| Workers | Content moderators from Kenya and other African countries working through a contractor |
| Issue | Outsourced content work, mental-health risk, termination, organizing, and lead-firm accountability |
| Worker action | Petition, legal challenge, and collective claims |
| Interim result | Trial-level protections were issued; Court of Appeal later set them aside |
| Final merits result | Not established by this judgment |
| Evidence class | Official appellate court record |
| Main uncertainty | Final disposition, compliance, worker outcomes, and the reach of any remedy |

## Relation to the ILO case table

These paired records update the Kenyan row in [the ILO social-dialogue case table](ilo-social-dialogue-case-table-v1.md). The [procedural timeline](kenya-content-moderation-procedural-timeline-v1.md) keeps the petition, interim orders, appeals, and unresolved merits separate. Together they make the case a contested pathway with different outcomes across different appeals, rather than a simple example of successful or unsuccessful worker organizing.

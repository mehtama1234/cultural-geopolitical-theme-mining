...
| Dated event → same-person care/debt/work context | [MEPS dated event-to-household-response screen](meps-dated-cascade-event-screen-v1.md) | Strict inter-round event windows now sit beside R4/2 care delay, bill problems, medical debt, collector contact, and nonemployment, with payment and BRR uncertainty kept on the event/person route. | Claim-level obligation, household adaptation timing, job-loss transition, remedy, and recovery |
| Dated event + institutional friction → same-person burden context | [MEPS dated friction cascade](meps-dated-friction-cascade-v1.md) | Within strict inter-round windows, denial/prior-authorization reporters show higher weighted care-delay, bill-problem, debt, and collector-contact shares across office, ER, and inpatient event families. | Round-level friction has no claim date or remedy; event causation, alternatives, obligation, and recovery remain open |
| Dated event → later health/work transition | [MEPS event six-month follow-up](meps-event-six-month-followup-v1.md) | Strict R3/1–R4/2 event windows carry same-person R4/2 baseline to R5/3 health and employment levels plus conditional onset/resolution transitions. | Later status is not verified recovery, remedy, treatment continuity, or proof that the event initiated the household episode |
| Event work level → later work clock | [MEPS event-to-work two-clock audit](meps-event-work-two-clock-audit-v1.md) | R4/2 nonemployment levels are placed beside R5/3 nonemployment, status change, onset, and resolution by event family; acute-event level differences persist while transition directions are mixed. | The two outputs are aligned by event rule, not row-merged; no job-loss cause, schedule/earnings trade-off, remedy, or recovery is observed |
| Institutional friction → later health/work transition | [MEPS friction-to-later-panel screen](meps-friction-later-panel-v1.md) | Within the same strict event window, `EQDENY53` groups can be carried to R5/3 health/work levels and baseline-conditioned transitions. | Denial has no claim date, response, remedy, or causal attribution; no-denial is not a counterfactual |

The strict-window screen also conditions the friction comparison on any private
coverage and low confidence paying an unexpected expense. The contrast remains
visible in the largest cells, but the room measures are annual/person-level
context and the small inpatient cells are not suitable for ranking.

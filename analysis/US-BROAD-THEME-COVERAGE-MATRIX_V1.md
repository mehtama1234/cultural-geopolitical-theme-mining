...
| Unequal exposure and status | [Unequal exposure/status layer](US-UNEQUAL-EXPOSURE-STATUS-LAYER_V1.md); SIPP tenure × resource estimates; USDA food security; ATUS time; discrimination, local-price, disability, age, and migration packets | Person, household, place, customer, worker, program episode | Compared / Open | Test whether exposure, treatment, remedy, and exit differ by intersecting status and available alternatives, not just by one demographic axis; complete one valid intersection with uncertainty and downstream outcome. |

...
Start with [the HTML guide](../site/us-theme-atlas.html) and its [Markdown edition](us-theme-atlas.md). Fifty-eight topics now sit under five shared themes, with eighty-four explained cross-topic connections. Each topic includes subthemes, sources and limits. Connections are labeled as comparisons or questions to test; they are not established causes.
...
Keep research passes short. Add a supported finding, a useful connection, or a clear unresolved question, then move to the next topic. The fifty-eight opening packets exist; the next work is checking their strongest claims and connections, not recreating project briefs.
